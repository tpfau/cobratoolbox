classdef (HandleCompatible) Node < handle
    ...
    methods
        ...
        function addChild(self,childNode)
            ...
            if isa(childNode,class(self)) %if the nodes are of the same class, we just add the children.
                for i=1:numel(childNode.children)
                    ...
                    cchild.parent = self.parent;
                    if isempty(self.children)
                        self.children = {cchild};
                    else                                          
                        self.children{end+1} = cchild;
                    end                    
        ...
        function removeDuplicateLiterals(self)
            ...
            % 
            if ~isa(self,'LiteralNode')
                literals = find(cellfun(@(x) isa(x,'LiteralNode'),self.children));
                literalIDs = cellfun(@(x) x.id,self.children(literals),'Uniform',false);
        ...
        %  
        tf = true;
        if isa(self,'LiteralNode') || all(cellfun(@(x) isa(x,'LiteralNode'),self.children)) 
            tf = false;
        end        
        end
        ...
        tf = false;
        if isa(self,'AndNode')
            if all(cellfun(@(x) isa(x,'LiteralNode'),self.children))
                tf = true;
            end
        elseif isa(self,'LiteralNode')
            tf = true;
        elseif isa(self,'OrNode')
            if all(cellfun(@(x) isa(x,'LiteralNode') || ( isa(x,'AndNode') && ~x.hasNestedChildren), self.children))
                tf = true;
            end
        end
        ...
        function [geneSets,genePos] = getFunctionalGeneSets(self, geneNames, getCNFSets)
            ...
            if isa(normNode,'LiteralNode')
                ...
                for i = 1:numel(normNode.children)
                    child = normNode.children{i};
                    childliterals = child.getLiterals();
